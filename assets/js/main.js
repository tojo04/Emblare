...
(function ($) {
    ...
    async function sendContactAsync() {
        ...
        try {
            ...
            formMessages.removeClass("error").addClass("success").text(text);

            // Clear the form inputs and textarea
            $(formElement).find('input:not([type="submit"]), textarea').val("");

        } catch (error) {
            formMessages.removeClass("success").addClass("error").text(
                error.message || "Oops! An error occured and your message could not be sent."
            );
        }
    }
